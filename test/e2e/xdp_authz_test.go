...
func applyYAML(manifest string) error {
	// Debug: show a preview of the manifest
	preview := manifest
	if len(preview) > 200 {
		preview = preview[:200] + "…"
	}
...
`, serverIP),
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Apply the policy
			t.Logf("Applying policy manifest:\n%s", tc.manifest)
			if err := applyYAML(tc.manifest); err != nil {
				t.Fatalf("apply policy %s failed: %v", tc.name, err)
			}
			defer deleteResource("authorizationpolicy", strings.Split(tc.manifest, "\n")[2])
			...
			if err != nil {
				t.Logf("Fortio command returned error: %v", err)
			}
