...
//  // waitDeploymentReady waits up to 60s for the named Deployment to have at least one available replica.
...
// 	 // 1) Fortio server Deployment: only HTTP on 8078, disable other listeners
...
// 	 // 3) Fortio client Deployment: use busybox so sleep works
...
// 	 // AuthorizationPolicy: deny by destination port 8078
...
// 	 // AuthorizationPolicy: deny by source IP
...
// 	 // AuthorizationPolicy: deny by destination IP (fixed schema)
...
// 	 // Helper: run a single Fortio HTTP request from the client Pod
